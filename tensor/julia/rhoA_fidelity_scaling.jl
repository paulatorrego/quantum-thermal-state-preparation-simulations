...
# ============================================================
# 12b. RIGOROUS LOCAL RHO_A + FIDELITY MODES
# ============================================================
# These additions are designed for the N-scaling campaign:
#   MODE=thermal_rhoA_ref  -> builds thermal local rho_A reference by purification MPS
#   MODE=rhoA_trace        -> runs trajectories, saves rho_A(t), F_A(t), D_A(t), observables
#
# Default block A is the central contiguous block of system qubits.
# RHOA_BLOCK_SIZE=1,2,3,4 are possible, but L=2 is the recommended default.
# The composite MPS site is system qubit ⊗ bath/ancilla qubit with basis:
#   1=|S0,B0>, 2=|S0,B1>, 3=|S1,B0>, 4=|S1,B1>.
# rho_A returned below traces out environment and bath/ancilla qubits, keeping
# only the system qubits in A.
# ============================================================

getenv_bool(name::String, default::Bool=false) = get(ENV, name, default ? "1" : "0") in ["1", "true", "TRUE", "yes", "YES"]

function rhoA_block_start(p::Params; L::Int=getenv_int("RHOA_BLOCK_SIZE", 2))
    start_env = getenv_int("RHOA_START", 0)
    if start_env > 0
        return start_env
    end
    # Central contiguous block. For even N and L=2 this gives N/2,N/2+1.
    return max(1, min(p.N - L + 1, div(p.N - L, 2) + 1))
end

@inline comp_system_index(a::Int) = div(a - 1, 2) + 1  # 1 or 2
@inline comp_bath_index(a::Int) = mod(a - 1, 2) + 1    # 1 or 2

function base_digits_1based(x::Int, base::Int, L::Int)
...
function pack_system_index_from_composite_digits(comp::Vector{Int})
    idx0 = 0
    mult = 1
    for a in comp
        idx0 += (comp_system_index(a) - 1) * mult
        mult *= 2
    end
    return idx0 + 1
end

function same_bath_digits(a::Vector{Int}, b::Vector{Int})
    @inbounds for k in eachindex(a)
        comp_bath_index(a[k]) == comp_bath_index(b[k]) || return false
    end
    return true
end

function sanitize_density_matrix(rho::Matrix{ComplexF64}; tol::Float64=1e-12)
    rho = ComplexF64.((rho + rho') ./ 2)
    trrho = real(tr(rho))
    if abs(trrho) > tol
        rho ./= trrho
    end
...
function local_system_rhoA_from_mps!(psi::MPS, sites, start::Int, L::Int)
    start < 1 && error("RHOA_START must be >=1")
    start + L - 1 > length(sites) && error("rhoA block exceeds system size: start=$start L=$L N=$(length(sites))")

    block_sites = collect(sites[start:(start + L - 1)])
    orthogonalize!(psi, start)

    W = psi[start]
    for j in (start + 1):(start + L - 1)
        W *= psi[j]
    end

    # Trace environment by contracting the canonical block boundary links.
    rho_comp_it = W * dag(prime(W, "Site"))
    primed_sites = [prime(s) for s in block_sites]
    arr = Array(rho_comp_it, vcat(block_sites, primed_sites)...)

    dsys = 2^L
    dcomp = 4^L
    rho_sys = zeros(ComplexF64, dsys, dsys)

    @inbounds for ket0 in 0:(dcomp - 1)
        ket_digits = base_digits_1based(ket0, 4, L)
        isys = pack_system_index_from_composite_digits(ket_digits)
        for bra0 in 0:(dcomp - 1)
            bra_digits = base_digits_1based(bra0, 4, L)
            same_bath_digits(ket_digits, bra_digits) || continue
            jsys = pack_system_index_from_composite_digits(bra_digits)
            rho_sys[isys, jsys] += arr[ket_digits..., bra_digits...]
        end
    end

    return sanitize_density_matrix(rho_sys)
end

function matrix_sqrt_psd(A::AbstractMatrix{<:Complex})
    H = Hermitian(ComplexF64.((A + A') ./ 2))
    F = eigen(H)
    vals = max.(real(F.values), 0.0)
    return F.vectors * Diagonal(sqrt.(vals)) * F.vectors'
end

function fidelity_density(rho::AbstractMatrix{<:Complex}, sigma::AbstractMatrix{<:Complex})
    rho2 = sanitize_density_matrix(Matrix{ComplexF64}(rho))
    sig2 = sanitize_density_matrix(Matrix{ComplexF64}(sigma))
    sr = matrix_sqrt_psd(rho2)
    M = sr * sig2 * sr
    val = real(tr(matrix_sqrt_psd(M)))^2
    return clamp(val, 0.0, 1.0 + 1e-10)
end

function trace_distance_density(rho::AbstractMatrix{<:Complex}, sigma::AbstractMatrix{<:Complex})
    Δ = Hermitian(ComplexF64.((rho - sigma + (rho - sigma)') ./ 2))
    return 0.5 * sum(abs.(real(eigvals(Δ))))
end

function rhoA_flat_namedtuple(rho::Matrix{ComplexF64}; prefix::String="rho")
    flat = vec(rho)
    names = Symbol[]
    vals = Any[]
    for k in eachindex(flat)
        push!(names, Symbol(prefix, "_re_", k)); push!(vals, real(flat[k]))
        push!(names, Symbol(prefix, "_im_", k)); push!(vals, imag(flat[k]))
    end
    return NamedTuple{Tuple(names)}(Tuple(vals))
end

function rhoA_from_csv_row(row; prefix::String="rho")
    dim = Int(row.rho_dim)
    vals = ComplexF64[]
    for k in 1:(dim * dim)
        push!(vals, ComplexF64(row[Symbol(prefix, "_re_", k)], row[Symbol(prefix, "_im_", k)]))
    end
    return sanitize_density_matrix(reshape(vals, dim, dim))
end

function load_rhoA_reference(path::String)
    isempty(strip(path)) && return nothing
    df = CSV.read(path, DataFrame)
    nrow(df) < 1 && error("Empty RHOA_REF_FILE=$path")
    rho = rhoA_from_csv_row(df[1, :]; prefix="rho")
    return rho
end

function physical_cycle_time_no_randomization(p::Params)
    # One reset/collision cycle applies the full filter sweep τ=-M,...,M.
    return length(filter_values(p)[1]) * p.delta
end

function obs_distance_to_reference(ob, target)
    terms = Float64[]
    for name in [:E, :MZ, :MX, :ZZ]
        ref = getfield(target, name)
        val = getfield(ob, name)
        if isfinite(ref) && isfinite(val)
            push!(terms, ((val - ref) / max(1.0, abs(ref)))^2)
        end
    end
    isempty(terms) && return NaN
    return sqrt(mean(terms))
end

# ---------- thermal rhoA reference by purification ----------

function product_mps_from_local_vectors_rhoA(sites, vecs::Vector{Vector{ComplexF64}})
    psi = MPS(sites)
...
function infinite_temperature_purification_rhoA(sites, p::Params)
    # Local thermofield pair |00> + |11> in physical ⊗ ancilla basis.
    v = ComplexF64[1, 0, 0, 1] ./ sqrt(2)
    return product_mps_from_local_vectors_rhoA(sites, [copy(v) for _ in 1:p.N])
end

function make_imag_gates_rhoA_ref(sites, p::Params, dtau::Float64)
    gates = ITensor[]
    Hloc = system_local_matrix(p)  # physical/system copy only: XS,ZS act on first qubit
...
function purification_rhoA_reference(p::Params; L::Int, start::Int, ntau::Int=getenv_int("NTAU", 1000))
    sites = siteinds("Qudit", p.N; dim=4)
    psi = infinite_temperature_purification_rhoA(sites, p)

    if p.beta > 0
        dtau = (p.beta / 2) / ntau
        gates = make_imag_gates_rhoA_ref(sites, p, dtau)
        @showprogress "thermal rhoA ref" for _ in 1:ntau
            psi = apply(gates, psi; cutoff=p.cutoff, maxdim=p.maxdim)
            normalize!(psi)
        end
    end

    rho = local_system_rhoA_from_mps!(psi, sites, start, L)
    ob = obs_summary!(psi, sites, p)
    return rho, ob, maximum(linkdims(psi))
end

function mode_thermal_rhoA_ref()
    p = params_from_env()
    L = getenv_int("RHOA_BLOCK_SIZE", 2)
    start = rhoA_block_start(p; L=L)
    ntau = getenv_int("NTAU", 1000)
    mkpath(p.outdir)

    println("Running THERMAL_RHOA_REF")
    println("  N=$(p.N), beta=$(p.beta), model=$(p.model), L=$L, start=$start, ntau=$ntau")
    println("  maxdim=$(p.maxdim), cutoff=$(p.cutoff)")

    rho, ob, bond = purification_rhoA_reference(p; L=L, start=start, ntau=ntau)
    dim = size(rho, 1)
    row = merge(
        param_row(p),
        (
            mode="thermal_rhoA_ref", rho_block_size=L, rho_start=start, rho_dim=dim,
            ntau=ntau, E=ob.E, MZ=ob.MZ, MX=ob.MX, XX=ob.XX, ZZ=ob.ZZ,
            BATH=ob.BATH, BOND=bond, trace_rho=real(tr(rho)), purity=real(tr(rho * rho)),
        ),
        rhoA_flat_namedtuple(rho; prefix="rho")
    )

    df = DataFrame([row])
    fname = joinpath(p.outdir, "$(p.out_prefix)_rhoA_ref_N$(p.N)_L$(L)_start$(start)_beta$(p.beta)_$(p.model).csv")
    CSV.write(fname, df)
    println("Saved rhoA thermal reference: $fname")
end

# ---------- trajectory trace saving rhoA(t) ----------

function run_rhoA_trace(p::Params; L::Int, start::Int, sample_start::Int, sample_stride::Int,
                        store_raw::Bool, rho_ref=nothing)
    target = reference_observables(p)
    sample_cycles = collect(sample_start:sample_stride:p.ncycles)
    ns = length(sample_cycles)
    dim = 2^L

    rho_sum = zeros(ComplexF64, dim, dim, ns)
    E_sum = zeros(Float64, ns); MZ_sum = zeros(Float64, ns); MX_sum = zeros(Float64, ns)
    XX_sum = zeros(Float64, ns); ZZ_sum = zeros(Float64, ns); BOND_sum = zeros(Float64, ns)
    MR_sum = zeros(Float64, ns); OUT_sum = zeros(Float64, ns)
    F_traj_sum = zeros(Float64, ns); D_traj_sum = zeros(Float64, ns); Dobs_sum = zeros(Float64, ns)
    n_count = zeros(Int, ns)

    raw_rows = DataFrame()
    cycle_time = physical_cycle_time_no_randomization(p)

    prog = Progress(p.ntraj * p.ncycles; desc="rhoA traj×cycle", dt=1.0)
    t0 = time()
    ...
    for r in 1:p.ntraj
        ...
        for c in 1:p.ncycles
            ...
            psi = maybe_extra_compress(psi, p, c)
            cum_MR += mr
            next!(prog; showvalues=[(:traj, r), (:cycle, c), (:N, p.N), (:bond, max_bond_dim(psi))])

            if c >= sample_start && ((c - sample_start) % sample_stride == 0)
                k = div(c - sample_start, sample_stride) + 1
                ob = obs_summary!(psi, sites, p)
                rho = local_system_rhoA_from_mps!(psi, sites, start, L)

                Ftraj = rho_ref === nothing ? NaN : fidelity_density(rho, rho_ref)
                Dtraj = rho_ref === nothing ? NaN : trace_distance_density(rho, rho_ref)
                Dobs = obs_distance_to_reference(ob, target)

                rho_sum[:, :, k] .+= rho
                E_sum[k] += ob.E; MZ_sum[k] += ob.MZ; MX_sum[k] += ob.MX
                XX_sum[k] += ob.XX; ZZ_sum[k] += ob.ZZ; BOND_sum[k] += ob.BOND
                MR_sum[k] += mr; OUT_sum[k] += mean(outcomes)
                isfinite(Ftraj) && (F_traj_sum[k] += Ftraj)
                isfinite(Dtraj) && (D_traj_sum[k] += Dtraj)
                isfinite(Dobs) && (Dobs_sum[k] += Dobs)
                n_count[k] += 1
                ...
                if store_raw
                    row = merge(
                        (
                            traj=r, cycle=c,
                            time_filter=c * cycle_time,
                            time_with_randomization=c * cycle_time + cum_MR * p.delta,
                            E=ob.E, MZ=ob.MZ, MX=ob.MX, XX=ob.XX, ZZ=ob.ZZ,
                            BATH=ob.BATH, BOND=ob.BOND, MR=mr, CUM_MR=cum_MR,
                            OUTCOME_mean=mean(outcomes), F_rhoA_traj=Ftraj,
                            trace_distance_rhoA_traj=Dtraj, D_obs=Dobs,
                        ),
                        rhoA_flat_namedtuple(rho; prefix="rho")
                    )
                    pushrow!(raw_rows, row)
                end
            end
        end
    end
    ...
    rows = DataFrame()
    for (k, c) in enumerate(sample_cycles)
        n = n_count[k]
        n == 0 && continue
        rho_mean = sanitize_density_matrix(rho_sum[:, :, k] ./ n)
        Fmean = rho_ref === nothing ? NaN : fidelity_density(rho_mean, rho_ref)
        Dmean = rho_ref === nothing ? NaN : trace_distance_density(rho_mean, rho_ref)
        row = merge(
            (
                cycle=c,
                time_filter=c * cycle_time,
                n_trajectory_blocks=n,
                E_mean=E_sum[k]/n, MZ_mean=MZ_sum[k]/n, MX_mean=MX_sum[k]/n,
                XX_mean=XX_sum[k]/n, ZZ_mean=ZZ_sum[k]/n, BOND_mean=BOND_sum[k]/n,
                MR_mean=MR_sum[k]/n, OUTCOME_mean=OUT_sum[k]/n,
                F_rhoA_mean=Fmean,
                infidelity_rhoA_mean=isfinite(Fmean) ? 1 - Fmean : NaN,
                trace_distance_rhoA_mean=Dmean,
                F_rhoA_traj_mean=(rho_ref === nothing ? NaN : F_traj_sum[k]/n),
                trace_distance_rhoA_traj_mean=(rho_ref === nothing ? NaN : D_traj_sum[k]/n),
                D_obs_mean=Dobs_sum[k]/n,
                E_ref=target.E, MZ_ref=target.MZ, MX_ref=target.MX, XX_ref=target.XX, ZZ_ref=target.ZZ,
            ),
            rhoA_flat_namedtuple(rho_mean; prefix="rho_mean")
        )
        pushrow!(rows, row)
    end

    elapsed = time() - t0
    return rows, raw_rows, elapsed
end

function mode_rhoA_trace()
    p = params_from_env()
    L = getenv_int("RHOA_BLOCK_SIZE", 2)
    start = rhoA_block_start(p; L=L)
    sample_start = getenv_int("RHOA_SAMPLE_START", 1)
    sample_stride = getenv_int("RHOA_SAMPLE_STRIDE", 1)
    store_raw = getenv_bool("RHOA_STORE_RAW", true)
    ref_file = getenv_str("RHOA_REF_FILE", "")
    rho_ref = load_rhoA_reference(ref_file)

    mkpath(p.outdir)
    println("Running RHOA_TRACE")
    println("  N=$(p.N), ntraj=$(p.ntraj), ncycles=$(p.ncycles), theta=$(effective_theta(p))")
    println("  block L=$L start=$start, sample_start=$sample_start, stride=$sample_stride")
    ref_file_label = isempty(ref_file) ? "NONE" : ref_file
    println("  ref_file=$ref_file_label")
    print_effective_params(p)

    rows, raw_rows, elapsed = run_rhoA_trace(
        p; L=L, start=start, sample_start=sample_start, sample_stride=sample_stride,
        store_raw=store_raw, rho_ref=rho_ref
    )

    base = "$(p.out_prefix)_rhoA_trace_N$(p.N)_L$(L)_start$(start)_theta$(round(effective_theta(p), sigdigits=6))"
    ts_file = joinpath(p.outdir, "$(base)_timeseries.csv")
    CSV.write(ts_file, rows)
    println("Saved rhoA timeseries: $ts_file")

    if store_raw
        raw_file = joinpath(p.outdir, "$(base)_raw_samples.csv")
        CSV.write(raw_file, raw_rows)
        println("Saved rhoA raw samples: $raw_file")
    end

    # Steady-state summary from final fraction of sampled points.
    ss_frac = getenv_float("RHOA_STEADY_FRAC", 0.8)
    if nrow(rows) > 0
        cmin = minimum(rows.cycle)
        cmax = maximum(rows.cycle)
        c0 = floor(Int, cmin + ss_frac * (cmax - cmin))
        ss = rows[rows.cycle .>= c0, :]
        summary = DataFrame([merge(
            param_row(p),
            (
                mode="rhoA_trace", rho_block_size=L, rho_start=start, rho_dim=2^L,
                sample_start=sample_start, sample_stride=sample_stride,
                n_samples=nrow(rows), ntraj=p.ntraj, walltime_sec=elapsed,
                steady_frac=ss_frac, steady_cycle_start=c0,
                F_rhoA_ss=mean(skipmissing(ss.F_rhoA_mean)),
                infidelity_rhoA_ss=mean(skipmissing(ss.infidelity_rhoA_mean)),
                trace_distance_rhoA_ss=mean(skipmissing(ss.trace_distance_rhoA_mean)),
                D_obs_ss=mean(skipmissing(ss.D_obs_mean)),
                E_ss=mean(skipmissing(ss.E_mean)), MZ_ss=mean(skipmissing(ss.MZ_mean)),
                MX_ss=mean(skipmissing(ss.MX_mean)), ZZ_ss=mean(skipmissing(ss.ZZ_mean)),
                BOND_peak=maximum(skipmissing(rows.BOND_mean)),
                has_rhoA_ref=(rho_ref !== nothing),
                rhoA_ref_file=ref_file,
            )
        )])
        sum_file = joinpath(p.outdir, "$(base)_summary.csv")
        CSV.write(sum_file, summary)
        println("Saved rhoA summary: $sum_file")
    end
end
...
function main()
    ...
    elseif mode == "steady"
        mode_steady()
    elseif mode == "thermal_rhoA_ref"
        mode_thermal_rhoA_ref()
    elseif mode == "rhoA_trace"
        mode_rhoA_trace()
    else
        error("Unknown MODE=$mode. Use quick, scan, convergence, theta, memory, variance, gibbs, steady, thermal_rhoA_ref, rhoA_trace.")
    end
end
